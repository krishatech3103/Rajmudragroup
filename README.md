...
1. Create a Supabase project and run [supabase_schema.sql](./supabase_schema.sql) in the SQL Editor.
   ...
   This is the only SQL file to run. It is rerunnable and preserves existing ledger rows. It includes database structure, login access control, permanent donation cleanup, Cash/UPI balance protection, and the linked FD flow.
...
2. The single [supabase_schema.sql](./supabase_schema.sql) script already removes anonymous database access and creates the `user_roles` table and RLS policies.
3. Assign roles in the Supabase SQL Editor, replacing the addresses below with your internal Auth addresses:

   ```sql
   INSERT INTO public.user_roles (user_id, role)
   SELECT id, 'admin' FROM auth.users WHERE email = 'admin@login.rajmudra.invalid'
   ON CONFLICT (user_id) DO UPDATE SET role = EXCLUDED.role, updated_at = NOW();

   INSERT INTO public.user_roles (user_id, role)
   SELECT id, 'viewer' FROM auth.users WHERE email = 'viewer@login.rajmudra.invalid'
   ON CONFLICT (user_id) DO UPDATE SET role = EXCLUDED.role, updated_at = NOW();
   ```
